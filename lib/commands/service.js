...
const PUBLIC_FIELDS = 'name id description package _user version url updatedAt createdAt'
...
const Commands = (service, models, client) => {
  ...
  const getById = id => models.Service.findById(id, PUBLIC_FIELDS)
    .catch(() => Promise.resolve(null))
    .then(ensureService)

  const update = (_id, data) => models.Service.findOneAndUpdate({
    _id
  }, data, { runValidators: true }).catch(error => utils.transformValidationErrors(error, service)).then(ensureService)
