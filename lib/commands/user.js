...
const Commands = (service, models, client) => {
  ...
  const update = (_id, data) => models.User.findOneAndUpdate({
    _id
  }, data, { runValidators: true, new: true }).catch(error => utils.transformValidationErrors(error, service)).then(ensureUser)
