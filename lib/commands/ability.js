...
const PUBLIC_FIELDS = 'name _module _user description event action state type format enum maxLength minLength pattern multipleOf minimum maximum exclusiveMaximum exclusiveMinimum updatedAt createdAt eventDescription actionDescription stateDescription'
...
const Commands = (service, models, client) => {
  ...
  const add = (userData, abilityData) => {
    return models.Module.findOne({
      _user: userData._id
    }).then(moduleData => {
      if (!moduleData) {
        return Promise.reject(new service.errors.Conflict(templates.userHasNotRelatedModule()))
      }
