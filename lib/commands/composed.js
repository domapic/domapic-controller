...
const Commands = (service, models, client, commands) => {
  ...
  const removeService = _id => {
    ...
    return commands.service.remove(_id)
      .then(() => commands.servicePluginConfig.findAndRemove(serviceQuery))
  ...
  const removeUser = _id => {
    ...
    return commands.user.remove({
      _id
    })
      .then(() => commands.securityToken.findAndRemove(userQuery))
      .then(() => commands.service.getFiltered(userQuery))
      .then(services => Promise.all(services.map(service => removeService(service._id))))
  }
