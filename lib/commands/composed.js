...
const Commands = (service, models, client, commands) => {
  ...
  const dispatchAbilityAction = (abilityId, data) => commands.ability.validateAction(abilityId, data)
    .then(ability => commands.module.getById(ability._module, {
      allFields: true
    }).then(moduleData => client.sendAction(moduleData, ability, data).then(() => commands.log.add({
  ...
  const getAbilityState = abilityId => commands.ability.validateState(abilityId)
    .then(ability => commands.module.getById(ability._module, {
      allFields: true
    }).then(moduleData => client.getState(moduleData, ability)))
